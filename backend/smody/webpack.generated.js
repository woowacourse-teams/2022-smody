...
const mavenOutputFolderForFlowBundledFiles = path.resolve(
  __dirname,
  "build/classes/META-INF/VAADIN/webapp"
);
const mavenOutputFolderForResourceFiles = path.resolve(
  __dirname,
  "build/classes/META-INF/VAADIN"
);
const useClientSideIndexFileForBootstrapping = true;
const clientSideIndexHTML = "./index.html";
const clientSideIndexEntryPoint = path.resolve(
  __dirname,
  "frontend",
  "generated/",
  "vaadin.ts"
...
// Flow plugins
const BuildStatusPlugin = require(buildDirectory +
  "/plugins/build-status-plugin");
const ThemeLiveReloadPlugin = require(buildDirectory +
  "/plugins/theme-live-reload-plugin");
const {
...
const projectStaticAssetsOutputFolder = path.resolve(
  __dirname,
  "build/classes/META-INF/VAADIN/webapp/VAADIN/static"
);
...
const swManifestTransform = manifestEntries => {
  ...
  if (useClientSideIndexFileForBootstrapping) {
    ...
    const indexEntryIdx = manifest.findIndex(
...
if (devMode) {
  ...
  const currentThemeFolders = [
    ...projectStaticAssetsFolders.map(folder =>
      path.resolve(folder, "themes", themeName)
    ),
  ...
  // to watch them.
  themeWatchFolders = [...currentThemeFolders, ...parentThemePaths].map(
    themeFolder => path.resolve(themeFolder, "components")
  );
}

const processThemeResourcesCallback = logger =>
  processThemeResources(themeOptions, logger);
          ...
          },
          {
            ...
            options: {
              url: (url, resourcePath) => {
                ...
                const themeResource =
                  resourcePath.match(themePartRegex) &&
                  url.match(/^themes\/[\s\S]*?\//);
            ...
            options: {
              ...
              name(resourcePath, resourceQuery) {
                if (resourcePath.match(/(\\|\/)node_modules\1/)) {
                  return /(\\|\/)node_modules\1(?!.*node_modules)([\S]+)/
                    .exec(resourcePath)[2]
                    .replace(/\\/g, "/");
                }
                if (resourcePath.match(/(\\|\/)flow-frontend\1/)) {
                  return /(\\|\/)flow-frontend\1(?!.*flow-frontend)([\S]+)/
                    .exec(resourcePath)[2]
                    .replace(/\\/g, "/");
                }
