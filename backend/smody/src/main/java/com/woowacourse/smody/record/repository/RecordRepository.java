...
public interface RecordRepository extends JpaRepository<Record, Long> {
    ...
    @Query(value = "select r.challenge_id from record r where r.member_id = :member and r.challenge_id in :challenges and r.dead_line >= :startTime", nativeQuery = true)
    ...
    @Query(value = "select r.challenge_id from record r where r.member_id = :member and r.challenge_id = :challenge and r.dead_line >= :startTime", nativeQuery = true)
    Long isInProgressSingleChallenge(@Param("member") Member member, @Param("challenge") Challenge challenge,
                                                         @Param("startTime") LocalDateTime startTime);

    @Query(value = "select r.challenge_id, count(r.member_id) from record r where r.challenge_id in :challenges and r.dead_line >= :startTime and r.is_success=false " +
